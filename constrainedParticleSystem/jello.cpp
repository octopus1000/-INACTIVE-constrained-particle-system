...
void display()
{
  ...
  gluLookAt(R * sin(Phi) * cos (Theta), R * sin (Theta) - 0.5, R * cos(Phi) * cos (Theta),
	        0.0,-.5,0.0, 0.0,1.0,0.);
  ...
  */
  ...
  // show the cube
  /*zhang: replace with show particle system*/
  /*showCube(&jello);*/
  showParticles(chain);
...
void doIdle()
{
  ...
  if (/*time > 1. / 15.*/saveScreenToFile==1)
  {
	  time = 0;
  ...
  if (pause == 0)
  {
    // insert code which appropriately performs one step of the cube simulation:
	  if (strcmp(chain.integrator, "Euler") == 0 || strcmp(chain.integrator, "euler") == 0)
		Euler(chain);
	  else
		RK4(chain);
	  inputf.x = inputf.y = 0;
...
int main (int argc, char ** argv)
{
  ...
  /*zhang:no need to read world*/
  /*readWorld(argv[1],&jello);*/

	if (argc<5)
	{
		printf("Oops! You didn't say the number of particles!\n");
		printf("Usage: %s [Euler or RK4] [number of particles] [alpha value] [beta value]\n", argv[0]);
		exit(0);
	}
